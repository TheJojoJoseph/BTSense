...
static void ICACHE_FLASH_ATTR resetTimerCb(void *arg) {
	...
	if (x==STATION_GOT_IP) {
		...
		wifi_set_opmode(1);
		os_sprintf((char*)sysCfg.sta_ssid,"%s",stconf.ssid);
		os_sprintf((char*)sysCfg.sta_pwd,"%s",stconf.password);
		os_sprintf((char*)sysCfg.configMode,"%s","0");
		CFG_Save();
		system_restart();
	} else {
